---
title: "04.源码级别Pod详解（一）：对象字段及源码实现"
description: "K8S使用的`API`风格为**声明式API** ，声明式API更加容易理解与维护，因为它们隐藏了底层的实现细节、我们通过`yaml`文件这种方式去创建、声明对应的对象，在K8S中的每一个对象都对应着一个`yaml`文件，即通过文件完成一次声明，而不是向命令式`API`一样通过一条条命令进行`API"
date: 2023-10-09
tags: ["K8S", "容器"]
---
...
K8S使用的`API`风格为**声明式API** ，声明式API更加容易理解与维护，因为它们隐藏了底层的实现细节、我们通过`yaml`文件这种方式去创建、声明对应的对象，在K8S中的每一个对象都对应着一个`yaml`文件，即通过文件完成一次声明，而不是向命令式`API`一样通过一条条命令进行`API`请求交互。
...
首先创建一个如下的`my-app-deployment.yaml`，这个`Deployment`旨在部署一个`pod`，这个`pod`只有一个容器，容器中运行着`nginx`服务。工作负载资源在创建`Pod`的时候，会按照`pod`模板里面的内容来创建实际的pod 。
[code]
    # 指定apiVersion
    apiVersion: apps/v1
    # 指定对象类型
...
![](https://cdn.nlark.com/yuque/0/2023/png/26372139/1698379855572-8d069924-2031-49b6-b17e-759adf0df2f9.png)

再通过`kubectl get pods -n default`来查看命名空间下的所有`pod`。

![](https://cdn.nlark.com/yuque/0/2023/png/26372139/1698379873283-beb9d7b4-2364-4583-adcc-573732b19cb7.png)
...
《每天十分钟，轻松入门K8S》的第四篇**04.源码级别Pod详解（一）** 到这里就结束了，感谢您看到这里。
